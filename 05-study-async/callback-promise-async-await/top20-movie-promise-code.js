...
axios
    .get(url)   // 2. GET 요청
    .then((result) => { // 결괏값 처리
        if(result.status != 200) {  // 상태가 200이 아니면 ERROR
        ...
        throw new Error(ERROR_DEFAULT); // data가 없으면 에러
    })
    .then((data) => {   // 4. 3에서 받은 데이터 처리
    ...
    .catch((error) => { // 8. 중간에 발생한 에러들을 여기서 처리
        console.log("<<에러 발생>>")
        console.error(error);
    })
